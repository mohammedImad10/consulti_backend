...
class AgenticRAG:
    ...
    def load_ingestion_metadata(self) -> dict:
        if os.path.exists(self.metadata_path):
            try:
                ...
                socketio.emit('message', f'[ERROR] Loading metadata failed: {e}')
        return {}
    ...
    def save_ingestion_metadata(self):
        try:
            ...
            socketio.emit('message', f'[ERROR] Saving metadata failed: {e}')
    ...
    async def extract_text_from_pdf(self, pdf_path: str) -> List[str]:
        try:
            ...
            socketio.emit('message', f'[INFO] Extracted {len(text_chunks)} chunks from PDF: {pdf_path}')
            return text_chunks
        except Exception as e:
            print(f"[ERROR] Failed to extract PDF {pdf_path}: {e}")
            socketio.emit('message', f'[ERROR] Failed to extract PDF {pdf_path}: {e}')
            return []
    ...
    async def extract_data_from_excel(self, excel_path: str) -> List[str]:
        try:
            ...
            socketio.emit('message', f'[INFO] Extracted {len(chunks)} chunks from Excel: {excel_path}')
            return chunks
        except Exception as e:
            print(f"[ERROR] Failed to extract Excel {excel_path}: {e}")
            socketio.emit('message', f'[ERROR] Failed to extract Excel {excel_path}: {e}')
            return []
    ...
    async def ingest_pdf(self, pdf_path: str) -> None:
        ...
        if pdf_path in self.ingestion_metadata and self.ingestion_metadata[pdf_path] >= mod_time:
            ...
            socketio.emit('message', f'[INFO] PDF unchanged: {pdf_path}')
            return
        text_chunks = await self.extract_text_from_pdf(pdf_path)
        if not text_chunks:
            print(f"[WARN] No chunks found in PDF: {pdf_path}")
            socketio.emit('message', f'[WARN] No chunks found in PDF: {pdf_path}')
            return
        ...
        socketio.emit('message', f'[INFO] Ingested PDF: {pdf_path} with {len(text_chunks)} chunks.')
    ...
    async def ingest_excel(self, excel_path: str) -> None:
        ...
        if excel_path in self.ingestion_metadata and self.ingestion_metadata[excel_path] >= mod_time:
            ...
            socketio.emit('message', f'[INFO] Excel unchanged: {excel_path}')
            return
        data_chunks = await self.extract_data_from_excel(excel_path)
        if not data_chunks:
            print(f"[WARN] No data extracted from Excel: {excel_path}")
            socketio.emit('message', f'[WARN] No data extracted from Excel: {excel_path}')
            return
        ...
        socketio.emit('message', f'[INFO] Ingested Excel: {excel_path} with {len(data_chunks)} chunks.')
    ...
    async def ingest_txt(self, txt_path: str) -> None:
        ...
        if txt_path in self.ingestion_metadata and self.ingestion_metadata[txt_path] >= mod_time:
            ...
            socketio.emit('message', f'[INFO] Text file unchanged: {txt_path}')
            return
        try:
            ...
            socketio.emit('message', f'[INFO] Ingested text file: {txt_path}')
        except Exception as e:
            print(f"[ERROR] Failed to ingest text file {txt_path}: {e}")
            socketio.emit('message', f'[ERROR] Failed to ingest text file {txt_path}: {e}')

    async def ingest_directory(self, directory: str) -> None:
        print(f"[INFO] Ingesting from directory: {directory}")
        socketio.emit('message', f'[INFO] Ingesting from directory: {directory}')
        for filename in os.listdir(directory):
            ...
            if os.path.isfile(file_path):
                ...
                if ext == "pdf":
                    ...
                    socketio.emit('message', f'[WARN] Skipped unsupported file: {filename}')
        print(f"[INFO] Finished ingesting from directory: {directory}")
        socketio.emit('message', f'[INFO] Finished ingesting from directory: {directory}')
    ...
    async def query_data(self, context: RunContext, user_query: str, n_results: int = 5) -> list:
        try:
            ...
            socketio.emit('message', f'[ERROR] Error retrieving data for "{user_query}": {e}')
            return []
...
class LLMClient:
    ...
    def query(self, prompt: str) -> str:
        try:
            ...
            socketio.emit('message', f'[ERROR] LLM Error: {e}')
            return None

# =================== CHAT INTERFACE ===================
def chat_interface(socketio):
    global_docs_dir = "global_docs"
    customer_docs_dir = "customer_data_docs"
    ...
    socketio.emit('message', f'[INFO] Finished ingesting global docs.')

    print("[INFO] Ingesting customer docs...")
    socketio.emit('message', f'[INFO] Ingesting customer docs...')
    ...
    socketio.emit('message', f'[INFO] Finished ingesting customer docs.')
...
class ExcelTemplateProcessor:
    ...
    def __init__(self, file_path: str, llm_client: LLMClient,
                 agentic_rag_yellow: AgenticRAG, agentic_rag_pink: AgenticRAG, socketio):
    ...
    def llm_query(self, prompt, model="gpt-4o-mini"):
        try:
            ...
            print(f"[ERROR] LLM Error: {e}")
            self.socketio.emit('message', f'[ERROR] LLM Error: {e}')
            return None
    ...
    async def fill_yellow_cells_from_docs(self, max_fields=6):
        global socketio
        processed_fields = 0
        for sheet_name, data in self.sheets_data.items():
            for cell in data['yellow']:
                if processed_fields >= max_fields:
                    ...
                    socketio.emit('message', f'[INFO] Processed maximum yellow cells.')
                    return
                ...
                socketio.emit('message', f'[INFO] Yellow cell {field_id}, Header="{header}"')
                ...
                if not final_answer or final_answer.lower() in ["n/a", "i don't know", "no context provided", "no information provided"]:
                    ...
                    print(  f"question: {type(question)}")
                    socketio.emit('message', json.loads(question))
                ...
                socketio.emit('message', f'[INFO] Yellow cell {field_id} set to "{cell.value}".\n\n')

    async def fill_pink_cells_from_docs(self, max_fields=6):
        processed_fields = 0
        for sheet_name, data in self.sheets_data.items():
            for cell in data['pink']:
                if processed_fields >= max_fields:
                    print("[INFO] Processed maximum pink cells.")
                    socketio.emit('message', f'[INFO] Processed maximum pink cells.')
                    return
                if cell.value and isinstance(cell.value, str) and cell.value.strip().lower() != "n/a":
                    continue
                processed_fields += 1
                ...
                socketio.emit('message', f'[INFO] Pink cell {field_id}, Header="{header}"')
                question = self.get_question_for_cell(cell, header, unit)
                ...
                socketio.emit('message', f'[INFO] Agentic RAG response: {response}')
                final_answer = self.parse_final_answer(response)
                if not final_answer or final_answer.lower() in ["n/a", "i don't know", "no context provided", "no information provided"]:
                    print(f"[WARN] No valid FINAL_ANSWER for {header} field {field_id}. Prompt user.\n")
                    print(  f"question: {question}")
                    print(  f"question: {type(question)}")
                    socketio.emit('message', json.loads(question))
                    # socketio.emit('message', {'question': question})
                    global response_event, user_response
                    response_event.clear()
                    print("Waiting for user response...")
                    await response_event.wait()
                    ...
                    socketio.emit('message', f'[INFO] User response in cell fun: {user_val}')
                    cell.value = self.convert_value(user_val) if user_val.strip() else "N/A"
                else:
                    cell.value = self.convert_value(final_answer)
                print(f"[INFO] Pink cell {field_id} set to '{cell.value}'.\n\n")
                socketio.emit('message', f'[INFO] Pink cell {field_id} set to "{cell.value}".\n\n')
...
def set_user_response(response, loop):
    ...
    socketio.emit('message', f'User response set: {user_response}')
...
def chat(agentic_rag_global, agentic_rag_customer):
    ...
    socketio.emit('message', '[INFO] Starting chat interface. Type "exit" to quit.')
    while True:
...
async def main(socket):
    ...
    # print("[INFO] Ingesting global docs...")
    # socketio.emit('message', '[INFO] Ingesting global docs...')
    agentic_rag_global = AgenticRAG(collection_name="global_agent_knowledge", is_client=False)
    # await agentic_rag_global.ingest_directory(global_docs_dir)
    # print("[INFO] Finished ingesting global docs.")
    # socketio.emit('message', {"question": '[INFO] Finished ingesting global docs.'})

    print("[INFO] Ingesting customer docs...")
    socketio.emit('message', '[INFO] Ingesting customer docs...')
    agentic_rag_customer = AgenticRAG(collection_name="customer_data_knowledge", is_client=True)
    await agentic_rag_customer.ingest_directory(customer_docs_dir)
    print("[INFO] Finished ingesting customer docs.")
    socketio.emit('message', '[INFO] Finished ingesting customer docs.')
    ...
    processor = ExcelTemplateProcessor("financial_sheet.xlsx", llm_client, agentic_rag_customer, agentic_rag_global, socketio)
